...
import com.contrastsecurity.cassandra.migration.internal.resolver.java.dummy.V2__InterfaceBasedMigration;
import com.contrastsecurity.cassandra.migration.internal.resolver.java.dummy.Version3dot5;
...
public class JavaMigrationResolverTest {
    ...
    public void broken() {
        new JavaMigrationResolver(Thread.currentThread().getContextClassLoader(), new ScriptsLocation("com/contrastsecurity/cassandra/migration/internal/resolver/java/error")).resolveMigrations();
    }

    @Test
    public void resolveMigrations() {
        JavaMigrationResolver jdbcMigrationResolver =
                new JavaMigrationResolver(Thread.currentThread().getContextClassLoader(), new ScriptsLocation("com/contrastsecurity/cassandra/migration/internal/resolver/java/dummy"));

...
class CassandraMigrationApiKIT : BaseKIT() {
    ...
    init {
        ...
        "Cassandra migration" - {
            ...
            // NOTE: Pending test due to issues with DataStax Queries not returning results within test, however the
            //       data exists within the test datastore. Needs further investigation.
            //
            //       For example:
            //       {{{
            //           val select = QueryBuilder.select().from("test2")
            //           val result = getSession().execute(select) // -> This returns result.exhausted == true
            //           result.one()
            //       }}}
...
//                        "3.0.1" -> {
//                            info.description shouldBe "Three point zero one"
...
//                            val result = getSession().execute(select)
//                            result.one().getString("value") shouldBe "facebook.com"
//                        }
//                        "3.0" -> {
...
//                            val result = getSession().execute(select)
//                            result.one().getString("value") shouldBe "google.com"
//                        }
//                        "2.0.0" -> {
...
//                            row.getString("message") shouldBe "bar"
//                        }
//                        "1.2.0" -> {
...
//                            val result = getSession().execute(select)
//                            result.one().getString("value") shouldBe "profit!"
//                        }
//                        "1.0.0" -> {
